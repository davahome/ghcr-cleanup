module.exports = async ({github, inputs}) => {
    ...
    if (!shouldDeleteUntagged) {
        ...
        const keepVersions = 0 + inputs.keep_versions_untagged;

        console.log("Checking package " + inputs.owner + "/" + inputs.package + " for untagged versions");
        console.log("Keeping (at least) " + keepVersions + " untagged versions");

        const response = await github.request("GET /" + inputs.owner + "/packages/container/" + inputs.package + "/versions", {
            per_page: 100
        });
        ...
        for (version of response.data) {
            ...
            const deleteResponse = await github.request("DELETE /" + inputs.owner + "/packages/container/" + inputs.package + "/versions/" + version.id, {});
